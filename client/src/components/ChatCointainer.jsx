...
import { toast } from 'react-toastify'; // Assuming you have toast imported for image handling
  ...
  return selectedUser ? (
    <div className='h-full overflow-hidden relative backdrop-blur-lg flex flex-col'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <img src={assets.help_icon} alt="Help" className='hidden md:block max-w-5' />
      </div>

      {/* ----------Chat Body-------------*/}
      <div className='flex-1 flex flex-col overflow-y-scroll p-3 pb-6'>
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 my-2 ${msg.senderId === authUser._id ? 'self-end' : 'self-start'}`}>
            <div className={`flex items-end gap-2 ${msg.senderId === authUser._id ? 'flex-row' : 'flex-row-reverse'}`}>
              ...
              {msg.image ? (
                ...
                <p className={`p-2 px-3 max-w-[250px] md:text-sm font-light rounded-lg break-words text-white ${msg.senderId === authUser._id ? 'bg-violet-500 rounded-br-none' : 'bg-gray-700 rounded-bl-none'}`}>{msg.text}</p>
              )}
              
              {/* Avatar and Time */}
              <div className='text-center text-xs'>
                <img src={msg.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="Sender" className='w-7 h-7 rounded-full' />
