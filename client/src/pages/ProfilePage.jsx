...
const ProfilePage = () => {
  ...
  // --- 3. Update submit handler to manage loading state ---
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedImg) {
        ...
        // This part is async, so we wrap it in a promise
      ...
      <>
        ...
        <div className="bg-[url('/bgImage.png')] bg-cover min-h-screen flex items-center justify-center p-4">
          <div className='w-full max-w-2xl bg-white/20 backdrop-blur-xl text-slate-700 border border-white/30 flex items-center justify-between max-sm:flex-col-reverse rounded-2xl relative shadow-2xl'>
            ...
            <div className="flex-shrink-0 p-10 max-sm:pt-10 max-sm:pb-0">
              <img className= "w-44 h-44 object-cover rounded-full mx-auto shadow-lg border-4 border-white/50" src= {selectedImg ? URL.createObjectURL(selectedImg) : (authUser?.profilePic || assets.avatar_icon)} alt="Current Profile" />
            </div>
          </div>
        </div>
      </>
